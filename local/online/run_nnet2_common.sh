...
if [ $stage -le 1 ]; then
  ...
  cat $data_dir/wav.scp | python2 -c "
...
for line in sys.stdin.readlines():
  if len(line.strip()) == 0:
    continue
  print 'echo {0} | sox --vol {1} -t wav - -t wav - |'.format(line.strip(), random.uniform(scale_low, scale_high))
"| sort -k1,1 -u  > $data_dir/wav.scp_scaled || exit 1;
  mv $data_dir/wav.scp_scaled $data_dir/wav.scp
  steps/make_mfcc_pitch_online.sh --nj 4 --mfcc-config conf/mfcc_hires.conf \
  ...
  steps/make_mfcc_pitch_online.sh --cmd "$train_cmd" --nj 4 --mfcc-config conf/mfcc_hires.conf \
...
if [ $stage -le 4 ]; then
  echo $stage
  cp -r data/train_scaled_hires_100k data/train_scaled_hires_100k_backup
  cp -r exp/nnet2_online/tri3b exp/nnet2_online/tri3b_backup
  cp -r exp/nnet2_online/diag_ubm exp/nnet2_online/diag_ubm_backup
  # iVector extractors can be sensitive to the amount of data, but this one has a
  # fairly small dim (defaults to 100) so we don't use all of it, we use just the
  # 100k subset (just under half the data).
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 2 \
    data/train_scaled_hires_100k_backup exp/nnet2_online/diag_ubm_backup exp/nnet2_online/extractor_backup || exit 1;
  exit
fi
