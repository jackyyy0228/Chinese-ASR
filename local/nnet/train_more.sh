...
# training scheduler,
learn_rate=0.004   # initial learning rate,
momentum=0
...
if [ $# != 5 ]; then
   echo "Usage: $0 <mlp-now> <data-train> <lang-dir> <ali-train> <exp-dir>"
...
local/nnet/train_more_scheduler.sh \
  ${scheduler_opts} \
  ...
  $mlp_init  "$feats_tr" "$labels_tr" $dir
