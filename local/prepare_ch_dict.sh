...
dict_dir=data/local/dict_ch_ABCD
...
if [ $stage -le 0 ] ; then
  ...
  for corpus in cyberon_chinese_train PTS NER seame Tl ; do
    cat ./data/$corpus/mfcc39_pitch9/text | awk '{for (i = 2; i <= NF; i++) print $i}' >> $lm_text
  done

  . local/data/corpus_path.sh
  PYTHONIOENCODING=utf-8 python3 local/data/extract_wiki.py $wiki >> $lm_text
  PYTHONIOENCODING=utf-8 python3 local/data/extract_ptt.py  $ptt >> $lm_text
fi
...
if [ $stage -le 1 ] ; then
  ...
  echo "A \n B \n C \n D \n" > $dict_dir/lexicon-en/words-en.txt
...
if [ $stage -le 5 ] ; then
  ...
  cat $dict_dir/lexicon-ch/lexicon-ch.txt | sed -e 's/U:/V/g' | sed -e 's/ R\([0-9]\)/ ER\1/g'| sed -e 's/ M\([0-9])/ N\1/g'|\
